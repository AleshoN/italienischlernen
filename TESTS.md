...
- GitHub-Actions-Prüfung – erfolgreich: Lauf `31058156908` installierte die Abhängigkeiten, bestand 25 Tests, ESLint, Prettier, Pages-Build und 23 Releaseprüfungen und veröffentlichte anschließend das Pages-Artefakt. Alle 13 regulären Workflow-Schritte und die Abschlussarbeiten wurden von GitHub als erfolgreich gemeldet.
- Öffentliche Browserprüfung – erfolgreich: `https://aleshon.github.io/italienischlernen/` zeigte Version 1.0.1, das Dashboard und alle 13 Kursmodule. Die öffentliche Wörterbuchansicht zeigte 17.106 kombinierte sowie 16.707 offene Einträge. Manifest, Symbol, CSS, JavaScript und Service Worker wurden unter `/italienischlernen/` geladen; Manifest-Start und -Gültigkeitsbereich waren relativ, die Cachekennung lautete `piano-app-v1.0.1-r1`. Es wurden keine Browserfehler protokolliert.
- Patch-Importtest – erfolgreich: Ein entpacktes Vollrelease 1.0.0 bestand den Trockenlauf und wurde anschließend mit `patch-1.0.0-to-1.0.1.zip` auf 1.0.1 aktualisiert. 25 Payload-Dateien wurden vorab per SHA-256 geprüft. Die alte JavaScript-Datei wurde entfernt, die neue eingesetzt, eine nicht zum Programm gehörende Testdatei blieb unverändert und die aktualisierte Installation bestand alle 23 Releaseprüfungen.
- Releasepaket-Prüfung – erfolgreich: Das entpackte Vollpaket 1.0.1 bestand alle 23 Releaseprüfungen. 113 Archiveinträge wurden kontrolliert; Pages-Workflow, Produktionsbuild, Wörterbuch, Releasehinweise und Quellcode sind enthalten. `sources/`, `node_modules/` und alte Patchverzeichnisse fehlen wie vorgesehen.

Der erste Aufruf der lokalen Prüfwerkzeuge konnte nicht starten, weil Node.js in dieser Sitzung noch nicht im sichtbaren Programmpfad lag. Nach Einbindung der vorhandenen Projektlaufzeit wurden alle oben aufgeführten Prüfungen neu und erfolgreich ausgeführt. Prüfungen auf physischen Mobilgeräten, mit einem echten Screenreader und bei manuell eingestelltem Browserzoom von 200 Prozent wurden für Version 1.0.1 nicht wiederholt.
